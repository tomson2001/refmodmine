...
if ( strtolower(substr($input, -5)) === ".bpmn" ) {
	$target = substr($input, 0, -5).".bpmn";
...
if ( is_null($outputFormat) ) {
	$_POST["msg"] = "<strong>An error occured. </strong> The input filetype could not be detected. Please do only use .bpmn files.";
	$_POST["msgType"] = "danger";
	Logger::log($email, "BPMN to EPML Conversion failed.  The input filetype could not be detected. Input: ".$input, "ERROR");
} else {
	$_POST["input"] = $input;
	$_POST["output"] = $output;
	$_POST["notification"] = $email;
	$actionHandler = new WorkspaceActionHandler();
	$actionHandler->run("CONVERT_BPMN2EPML");
}
sleep(1);
?>