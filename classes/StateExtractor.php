<?php
/**
 * Klasse zur Extraktion eines ReachabilityGraphs.
 *
 * Zulaessig sind:
	...
	public function execute() {
		...
		if ( count($startNodes) == 1 || $firstCommonJoin == "xor" || $firstCommonJoin == "or" ) {
			...
			foreach ( $startNodes as $startNodeId => $label ) {
				...
				$state = $this->reachabilityGraph->addState($state);

				$trace = array($startNodeId, $successor);
		...
		if ( count($startNodes) > 1 ) {
			...
			if ( $firstCommonJoin == "and" || $firstCommonJoin == "or" ) {
				...
				foreach ( $startNodes as $startNodeId ) {
					...
					$startNodes_tmp = $startNodes;
					unset($startNodes_tmp[$startNodeId]);
					$startNodes_tmp[$successor] = $this->epc->getNodeLabel($successor);
					...
					$traceExtraction = array( // detection
							array(
									...
									'backEdges' => array(),
									'connectorMemory' => array(),
									'todoMemory' => array("start" => array_keys($startNodes_tmp)),
			...
			if ( $countStartNodes > 2 ) {
				...
				for ( $i=2; $i<$countStartNodes; $i++ ) {
					...
					foreach ( $possibleTodoMemories as $todoMemory ) {
						...
						$tokens = array();
						$todoMemorySuccessors = array();
						foreach ( $todoMemory as $permNodeId ) {
							$permNodeSuccs = $this->epc->getSuccessor($permNodeId);
							$permNodeSucc = $permNodeSuccs[0];
							array_push($todoMemorySuccessors, $permNodeSucc);
							array_push($tokens, array($startNodeId => $permNodeSucc));
						}
						// Zustand erstellen
						$state = new State($tokens);
						$existingState = $this->reachabilityGraph->containsState($state);
						$state = $this->reachabilityGraph->addState($state);
						...
						if ( $firstCommonJoin == "and" || $firstCommonJoin == "or" ) {
							...
							$traceExtraction = array( // detection
									array(
											...
											'backEdges' => array(),
											'connectorMemory' => array(),
											'todoMemory' => array("start" => $todoMemorySuccessors),
											'todoMemorySubTraces' => array("start" => array()),
			...
			*/
			if ( $currentNodeType == "event" || $currentNodeType == "function" ||
				...
				if ( count($successors) == 1 ) {
					...
					// Zustand erstellen
					$state = new State(array($currentNode => $successor));
					$existingState = $this->reachabilityGraph->containsState($state);
					if ( $existingState === false ) {
						$state = $this->reachabilityGraph->addState($state);
						$this->reachabilityGraph->addTransition($newDetection['lastState'], $state, $this->epc->getNodeLabel($successor));
						$newDetection['lastState'] = $state;
					} else {
						$this->reachabilityGraph->addTransition($newDetection['lastState'], $existingState, $this->epc->getNodeLabel($successor));
						continue;
					}
			...
			if ( $this->epc->isANDSplit($currentNode) || $currentNodeType == "or" ) {
				...
				foreach ($successors as $successor) {
					...
					array_push($tokens, array($currentNode => $successor));
				...
				if ( $countSucc > 2 ) {
					...
					for ( $i=2; $i<$countSucc; $i++ ) {
						...
						foreach ( $possibleTodoMemories as $todoMemory ) {
							...
							$tokens = array();
							foreach ( $todoMemory as $permNodeId ) {
								array_push($tokens, array($currentNode => $permNodeId));
							}
							// Zustand erstellen
							$state = new State($tokens);
							$existingState = $this->reachabilityGraph->containsState($state);
					...
					if ( substr($node, 0, 8) == 'stop_and' ) {
						...
						if ( $countInputPathes == $foundPathes ) {
							...
							if ( count($successors) == 1 ) {
								...
								// Zustand erstellen
								$state = new State(array($andJoinID => $successor));
								$existingState = $this->reachabilityGraph->containsState($state);
								if ( $existingState === false ) {
									$state = $this->reachabilityGraph->addState($state);
									$this->reachabilityGraph->addTransition($newDetection['lastState'], $state, $this->epc->getNodeLabel($andJoinID));
									$newDetection['lastState'] = $state;
								} else {
									$this->reachabilityGraph->addTransition($newDetection['lastState'], $existingState, $this->epc->getNodeLabel($andJoinID));
									continue;
								}
						...
						// Zustand erstellen
						$state = new State(array($newDetection['currentNode'] => $node));
						$existingState = $this->reachabilityGraph->containsState($state);
						if ( $existingState === false ) {
							$state = $this->reachabilityGraph->addState($state);
							$this->reachabilityGraph->addTransition($newDetection['lastState'], $state, $this->epc->getNodeLabel($node));
							$newDetection['lastState'] = $state;
						} else {
							$this->reachabilityGraph->addTransition($newDetection['lastState'], $existingState, $this->epc->getNodeLabel($node));
							$newDetection['lastState'] = $existingState;
						}
			...
			foreach ( $possibleNodes as $index => $nodeID ) {
				...
				if ( substr($nodeID, 0, 7) == 'stop_or' ) {
					...
					if ( $allPathesReached || !$activatedPathes ) {
						...
						// Zustand erstellen
						$state = new State(array($orJoinID => $successor));
						$existingState = $this->reachabilityGraph->containsState($state);
						if ( $existingState === false ) {
							$state = $this->reachabilityGraph->addState($state);
							$this->reachabilityGraph->addTransition($newDetection['lastState'], $state, $this->epc->getNodeLabel($orJoinID));
							$newDetection['lastState'] = $state;
						} else {
							$this->reachabilityGraph->addTransition($newDetection['lastState'], $existingState, $this->epc->getNodeLabel($orJoinID));
							continue;
						}
				...
				if ( $nextNodeType == "event" || $nextNodeType == "function" ||	array_key_exists($nodeID, $joinConnectors) ) {
					...
					if ( count($successors) == 1 ) {
						...
						if ( !($splitNode == "start" && $this->epc->isXORJoin($nodeID))
							...
							// Zustand erstellen
							$state = new State($newDetection['trace']);
							$existingState = $this->reachabilityGraph->containsState($state);
							if ( $existingState === false ) {
								$state = $this->reachabilityGraph->addState($state);
								$this->reachabilityGraph->addTransition($newDetection['lastState'], $state, $this->epc->getNodeLabel($nodeID));
								$newDetection['lastState'] = $state;
							} else {
								$this->reachabilityGraph->addTransition($newDetection['lastState'], $existingState, $this->epc->getNodeLabel($nodeID));
								continue;
							}
