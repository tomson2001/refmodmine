...
// 		$string = str_replace("ß", "ss", $string);
	 ...
	 *  7   Split (X)OR cannot be predeccessors of events, since event have no decision character 
	 *  8   Events and activities should be atomar. E.g. "count and evaluatate" would be invalid
	 *  9   OR-connectors should be prevented
	 * 
	 * @return array
	 */
	public function getSyntaxErrors($errorTypes=array(1,2,3,6,7), $warningTypes=array(4,5)) {
		$issues = array();
		
		// Es existiert mind. ein Start und Endereignis
		if ( count($this->getAllStartNodes()) == 0 ) {
			if ( in_array(1, $errorTypes) ) array_push($issues, "SYN01 - Error: Start event missing.");
			if ( in_array(1, $warningTypes) ) array_push($issues, "SYN01 - Warning: Start event missing.");
		}
		if ( count($this->getAllEndNodes()) == 0 ) {
			if ( in_array(1, $errorTypes) ) array_push($issues, "SYN01 - Error: End event missing.");
			if ( in_array(1, $warningTypes) ) array_push($issues, "SYN01 - Warning: End event missing.");
		}
		...
		foreach ( $this->events as $eventID => $label ) {
			...
			if ( count($this->getPredecessor($eventID)) > 1 ) {
				if ( in_array(3, $errorTypes) ) array_push($issues, "SYN05 - Error: Event \"".$label."\" has more than one incoming edge.");
				if ( in_array(3, $warningTypes) ) array_push($issues, "SYN05 - Warning: Event \"".$label."\" has more than one incoming edge.");
			}
			if ( count($this->getSuccessor($eventID)) > 1 ) {
				if ( in_array(3, $errorTypes) ) array_push($issues, "SYN05 - Error: Event \"".$label."\" has more than one outgoing edge.");
				if ( in_array(3, $warningTypes) ) array_push($issues, "SYN05 - Warning: Event \"".$label."\" has more than one outgoing edge.");
			}
			...
			if ( !in_array($label, $eventLabels) ) {
				...
				if ( in_array(5, $errorTypes) ) {
					$error = "SEM04 - Error: Event \"".$label."\" occurs more than once.";
					if ( !in_array($error, $issues) ) array_push($issues, $error);
				}
				if ( in_array(5, $warningTypes) ) {
					$error = "SEM04 - Warning: Event \"".$label."\" occurs more than once.";
					if ( !in_array($error, $issues) ) array_push($issues, $error);
				}
			}
			...
			foreach ( $successors as $succID ) {
				if ( $this->isEvent($succID) ) {
					if ( in_array(4, $errorTypes) ) array_push($issues, "SYN02 - Error: Event \"".$label."\" is followed by another event.");
					if ( in_array(4, $warningTypes) ) array_push($issues, "SYN02 - Warning: Event \"".$label."\" is followed by another event.");
				}
			}
		}
		...
		foreach ( $this->functions as $funcID => $label ) {
			...
			if ( count($this->getPredecessor($funcID)) != 1 ) {
				if ( in_array(2, $errorTypes) ) array_push($issues, "SYN05 - Error: Function \"".$label."\" has not exactly one incoming edge.");
				if ( in_array(2, $warningTypes) ) array_push($issues, "SYN05 - Warning: Function \"".$label."\" has not exactly one incoming edge.");
			}
			if ( count($this->getSuccessor($funcID)) != 1 ) {
				if ( in_array(2, $errorTypes) ) array_push($issues, "SYN05 - Error: Function \"".$label."\" has not exactly one outgoing edge.");
				if ( in_array(2, $warningTypes) ) array_push($issues, "SYN05 - Warning: Function \"".$label."\" has not exactly one outgoing edge.");
			}
			...
			if ( !in_array($label, $funcLabels) ) { 
				...
				if ( in_array(5, $errorTypes) ) {
					$error = "SEM04 - Error: Function \"".$label."\" occurs more than once.";
					if ( !in_array($error, $issues) ) array_push($issues, $error);
				}
				if ( in_array(5, $warningTypes) ) {
					$error = "SEM04 - Warning: Function \"".$label."\" occurs more than once.";
					if ( !in_array($error, $issues) ) array_push($issues, $error);
				}
			}
			...
			foreach ( $successors as $succID ) {
				if ( $this->isFunction($succID) ) {
					if ( in_array(4, $errorTypes) ) array_push($issues, "SYN02 - Error: Function \"".$label."\" is followed by another function.");
					if ( in_array(4, $warningTypes) ) array_push($issues, "SYN02 - Warning: Function \"".$label."\" is followed by another function.");
				}
			}
		}
		...
		foreach ( $connectors as $connID => $type ) {
			...
			if ( count($this->getPredecessor($connID)) == 0 || count($this->getSuccessor($connID)) == 0 ) { 
				if ( in_array(6, $errorTypes) ) array_push($issues, "SYN04 - Error: Missing incoming and outgoing edges for ".$type."-connector.");
				if ( in_array(6, $warningTypes) ) array_push($issues, "SYN04 - Warning: Missing incoming and outgoing edges for ".$type."-connector.");
				$reported = true; 
			}
			if ( count($this->getPredecessor($connID)) <= 1 && count($this->getSuccessor($connID)) <= 1 && !$reported ) {
				if ( in_array(6, $errorTypes) ) array_push($issues, "SYN04 - Error: Missing incoming or outgoing edges for ".$type."-connector.");
				if ( in_array(6, $warningTypes) ) array_push($issues, "SYN04 - Warning: Missing incoming or outgoing edges for ".$type."-connector.");
			}
			if ( count($this->getPredecessor($connID)) > 1 && count($this->getSuccessor($connID)) > 1 ) {
				if ( in_array(6, $errorTypes) ) array_push($issues, "SYN04 - Error: ".$type."-connectors is split and join connector at the same time.");
				if ( in_array(6, $warningTypes) ) array_push($issues, "SYN04 - Warning: ".$type."-connectors is split and join connector at the same time.");
			}
			
			// @todo the following two checks must be automated by firstly checking the sample solution. Check, which connector are available and prevent that here.
			// OR-connectors should be prevented
			if ( $this->getType($connID) == "or" ) {
				if ( in_array(9, $errorTypes) ) array_push($issues, "SEM02 - Error: ".$type."-connectors should be prevented.");
				if ( in_array(9, $warningTypes) ) array_push($issues, "SEM02 - Warning: ".$type."-connectors should be prevented.");
			}
			
			// AND-connectors should be prevented / DELFI-Spec
			if ( $this->getType($connID) == "and" ) {
				if ( in_array(9, $errorTypes) ) array_push($issues, "SEM02 - Error: ".$type."-connector invalid (DELFI).");
				if ( in_array(9, $warningTypes) ) array_push($issues, "SEM02 - Warning: ".$type."-connector invalid (DELFI).");
			}
			
			if ( $this->getType($connID) == "xor" || $this->getType($connID) == "or" ) {
				
				$pred = $this->getPredecessor($connID);
				$succ = $this->getSuccessor($connID);
				if ( count($succ) > 1 && count($pred) == 1 && $this->getType($pred[0]) == "event" ) {
					if ( in_array(7, $errorTypes) ) array_push($issues, "SYN06 - Error: ".$type."-splits must not follow an event.");
					if ( in_array(7, $warningTypes) ) array_push($issues, "SYN06 - Warning: ".$type."-splits must not follow an event.");
				}
			}
		}
		
		// Abstraktionsgrad, Ereignisse und Funktionen sollen atomar sein, also genau ein Ereignis bzw. Aktivität bezeichnen
		foreach ( $this->functions as $id => $label ) {
			if ( substr_count($label, " + ") ) {
				if ( in_array(8, $errorTypes) ) array_push($issues, "SEM04 - Error: Several activities mixed in \"".$label."\".");
				if ( in_array(8, $warningTypes) ) array_push($issues, "SEM04 - Warning: Several activities mixed in \"".$label."\".");
			}
		}
		foreach ( $this->events as $id => $label ) {
			if ( substr_count($label, " + ") ) {
				if ( in_array(8, $errorTypes) ) array_push($issues, "SEM04 - Error: Several events mixed in \"".$label."\".");
				if ( in_array(8, $warningTypes) ) array_push($issues, "SEM04 - Warning: Several events mixed in \"".$label."\".");
			}
		}
		
// 		$splits = $this->getAllSplitConnectors($this->or+$this->xor);
// 		print("\n".count($splits)." relevant xor/or splits in ".$this->name." (".count($connectors).")\n");
	...
	public function autocorrectSpelling() {
		...
		// split joined functions
		foreach ( $this->functions as $id => $label ) {
			
			if ( substr_count($label, " + ") ) {
				
				$funcs = explode(" + ", $label);
				$numParts = count($funcs);
				
				$pred = $this->getPredecessor($id);
				$pred = $pred[0];
				$succ = $this->getSuccessor($id);
				$succ = $succ[0];
				
				$andSplitID = $this->getFreeNodeID();
				$this->and[$andSplitID] = "and";
				$this->addEdge($pred, $andSplitID);
				
				$andJoinID = $this->getFreeNodeID();
				$this->and[$andJoinID] = "and";
				$this->addEdge($andJoinID, $succ);
				
				$this->functions[$id] = $funcs[0];
				unset($funcs[0]);
				
				foreach ( $funcs as $funcPart ) {
					$newFuncID = $this->getFreeNodeID();
					$this->functions[$newFuncID] = $funcPart;
					$this->addEdge($andSplitID, $newFuncID);
					$this->addEdge($newFuncID, $andJoinID);
				}
				
				array_push($this->spellingCorrections, "'".$label."' splitted into  '".$numParts."' functions");
				
			}
			
		}
		
		$someFullCorrections = array(
			"ritten rejection created"	=> "rejection created",
			"cheack feasibility" => "check feasibility",
			"clarification"	=> "bring about clarification",
			"inform"	=> "inform customer",
			"inform c."	=> "inform customer",
			"rejection"	=> "create rejection",
			"offer"	=> "create offer"
		);
		
		$somePartialCorrections = array(
			"written "	=> ""
		);
		
		foreach ( $this->functions as $id => $label ) {
			
			foreach ( $somePartialCorrections as $search => $replace ) {
				if ( substr_count($label, $search) ) {
					$this->functions[$id] = str_replace($search, $replace, $label);
					array_push($this->spellingCorrections, "'".$label."' replaced by '".$this->functions[$id]."'");
				}
			}
			
			$label = $this->functions[$id];
			
			if ( isset($someFullCorrections[(string) $label]) ) {
				$this->functions[$id] = $someFullCorrections[$label];
				array_push($this->spellingCorrections, "'".$label."' replaced by '".$someFullCorrections[$label]."'");
			}
	...
	public function isNodeIDAssigned($id) {
		...
		if ( isset($this->functions[$id]) ) return $this->functions[$id];
