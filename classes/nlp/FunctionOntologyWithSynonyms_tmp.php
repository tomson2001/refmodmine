...
//  			"to", "and", "or", "of", "for", "by", "is", "if", "then", "else", "is", "are",
//  			"the", "with", "on", "has", "have", "about", "above", "within", "top", 
//  			"a", "as", "in", "other", "out"
 	);
	...
	private function buildOntology() {
		...
		foreach ( $label_components as $component ) {
			...
			if ( $this->useSynonyms ) {
				...
				if ( !empty($syns) ) {
					//$wordstem = $syns[0];
					$this->nouns[$wordstem] = array($wordstem);
					$this->synonyms[$wordstem] = array($wordstem);
				...
				if ( !empty($syns) ) {
					...
					$this->verbs[$wordstem] = array($wordstem);
					if ( !isset($this->synonyms[$wordstem]) ) $this->synonyms[$wordstem] = array($wordstem);
...
// 		if ( $this->label == "assessment of application" || $this->label == "Evaluate") {
// 			print_r($this->synonyms);
// 		}
