...
class StanfordPOSTagger {
	...
	// //////////////////////////////////////////////////////////////////////////
	/*
	 * english-bidirectional-distsim.tagger Trained on WSJ sections 0-18 using a bidirectional architecture and including word shape and distributional similarity features. Penn Treebank tagset.
	 */
	// private $model = 'english-bidirectional-distsim.tagger'; // 97.32% accuracy - slow
	/*
	 * english-left3words-distsim.tagger Trained on WSJ sections 0-18 and extra parser training data using the left3words architecture and includes word shape and distributional similarity features. Penn tagset.
	 */
		...
		foreach ( $sentences as $k => $v ) {
			...
			foreach ( $tags as $kk => $vv ) {
				...
				if ($this->sequence_tags) {
					if ( isset($sequence[$parts[1]]) && $sequence[$parts[1]] > 0) {
						$sequence [$parts [1]] ++;
					} else {
						$sequence [$parts [1]] = 1;
					}
	...
	public function spellcheck($words) {
		$o = '';
		if (function_exists ( "pspell_new" )) {
			$pspell_link = pspell_new ( "en" );
			foreach ( $words as $k => $v ) {
				if (! pspell_check ( $pspell_link, $v )) {
					$suggestion = pspell_suggest ( $pspell_link, $v );
					if ( !empty($suggestion) ) {
						$o .= $suggestion[0] . ' ';
					} else {
						$o .= $v. ' ';
